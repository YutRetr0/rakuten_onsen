...
def _wecom_bot(title, body, url):
    ...
    r = requests.post(hook, json={"msgtype": "markdown",
                                  "markdown": {"content": md}}, timeout=10)
    r.raise_for_status()
...
def _email(title, body, url):
    ...
    msg["Subject"] = title
    msg["From"] = formataddr(("Onsen Watcher", user)); msg["To"] = to
...
def _webhook(title, body, url):
    ...
    r = requests.post(hook, json={"title": title, "body": body, "url": url}, timeout=10)
    r.raise_for_status()
...
def _bark(title, body, url):
    ...
    r = requests.post(f"https://api.day.app/{{key}}",
                      json={"title": title, "body": body, "url": url}, timeout=10)
    r.raise_for_status()